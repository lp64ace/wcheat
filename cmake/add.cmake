...
function(wcheat_target_include_dirs_impl
	target
	system
	includes
)
...
function(wcheat_target_include_dirs
	target
)
	absolute_include_dirs(_ALL_INCS ${ARGN})
	wcheat_target_include_dirs_impl(${target} FALSE "${_ALL_INCS}")
endfunction()

function(wcheat_target_include_dirs_sys
	target
)
	absolute_include_dirs(_ALL_INCS ${ARGN})
	wcheat_target_include_dirs_impl(${target} TRUE "${_ALL_INCS}")
endfunction()
...
function(wcheat_user_header_search_paths
	name
	includes
)
	if(XCODE)
...
function(wcheat_target_link_libraries
	target
	library_deps
)
...
function(wcheat_add_lib__impl
	...
	wcheat_target_include_dirs(${name} ${includes})
	wcheat_target_include_dirs_sys(${name} ${includes_sys})

	if(library_deps)
		wcheat_target_link_libraries(${name} "${library_deps}")
	endif()

	# works fine without having the includes
	# listed is helpful for IDE's (QtCreator/MSVC)
	wcheat_source_group("${name}" "${sources}")
	wcheat_user_header_search_paths("${name}" "${includes}")
...
function(wcheat_add_lib
	...
	wcheat_add_lib__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")

	set_property(GLOBAL APPEND PROPERTY wcheat_LINK_LIBS ${name})
endfunction()

# only MSVC uses SOURCE_GROUP
function(wcheat_add_dll__impl
	name
	sources
	includes
	includes_sys
...
function(wcheat_add_dll
	...
	wcheat_add_dll__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")
...
function(wcheat_add_test_executable name sources includes includes_sys library_deps)
	add_cc_flags_custom_test(${name} PARENT_SCOPE)
	...
	wcheat_target_include_dirs(${target_name} includes)
	wcheat_target_include_dirs_sys(${target_name} includes_sys)
	
	target_link_libraries(${target_name} wcheat::intern::testing)
