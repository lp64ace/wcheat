...
int main(int argc, char **argv) {
	...
	do {
		if (_stricmp(entry.szExeFile, who) == 0) {
			...
			HMODULE hRemoteLibrary = GetRemoteModuleHandle(entry.th32ProcessID, dll);
			if (!hRemoteLibrary) {
				fprintf(stderr, "[Error] Failed to find loaded DLL!\n");
				return 0xdeadbeef;
			}
			...
			if (!dwResult) {
				fprintf(stderr, "[Error] Failed to unload DLL!\n");
				// return 0xdeadbeef;
			}
			CloseHandle(hThread);
